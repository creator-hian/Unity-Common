...
namespace Creator_Hian.Unity.Common
{
    ...
    {
        private static readonly Dictionary<string, FileTypeDefinition> TypesByExtension = new(
            StringComparer.OrdinalIgnoreCase
        );

        private static readonly Dictionary<
            FileCategory,
            HashSet<FileTypeDefinition>
        > TypesByCategory = new();
        ...
        public static FileTypeDefinition GetByExtension(string extension)
        {
            ...
            return TypesByExtension.TryGetValue(extension, out FileTypeDefinition type)
                ? type
        ...
        {
            return TypesByCategory.TryGetValue(category, out HashSet<FileTypeDefinition> types)
                ? types
        ...
        {
            FileTypeDefinition fileType = GetByExtension(extension);
        ...
        {
            return TypesByMimeType.TryGetValue(mimeType, out HashSet<FileTypeDefinition> types)
                ? types
                : EmptySet;
        }
        ...
        /// </remarks>
        [Obsolete(
            "This method is intended for testing purposes only. Do not use in production code."
        )]
